...
#Assigning column header names for covid_cases variable(united_states) 
colnames(covid_cases)[1] = "NAME" #name of states in US
colnames(covid_cases)[2] = "Total_case" # Total Cases for each state


total_cases <- sum(covid_cases$Total_case) #Summing to get total case for US
total_death <- sum(covid_cases$`Total Deaths`)#Summing to get total deaths for US
